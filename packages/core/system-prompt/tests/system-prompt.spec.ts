...
import SystemPrompt, {
  AssembleContext, MANGABA_IDENTITY, PromptAssembly, renderContextSnapshot, renderPrompt,
...
const IDENTITY = MANGABA_IDENTITY
...
describe('SystemPrompt', () => {
  ...
  describe('built-in sections', () => {
    it('registers the harness identity and the configured deployment persona', async () => {
      const ctx = new Context()
      await ctx.plugin(SystemPrompt, { persona: 'You are Mangaba Harness.' })
  ...
  it('assembles sections in order with context-resolved text and collected tools', async () => {
    ...
    expect(assembly.sections.map(s => s.text)).toEqual([IDENTITY, 'You are Mangaba Harness.', 'Be precise.', 'cwd: /tmp'])
    expect(assembly.contexts).toEqual([
