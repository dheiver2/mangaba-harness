...
export const zh = {
  ...
  'error.MISSING_CREDENTIAL': '该 provider 还没有 API key。打开 设置 → 模型 填写一个。',
  'error.INVALID_CREDENTIAL': 'provider 拒绝了这个 API key。请在 设置 → 模型 中重新填写。',
  'error.UNKNOWN_MODEL': '所选模型未配置。请在输入框的模型选择器中换一个。',
  'error.TRANSPORT': '无法连接到 provider。本地模型请确认 Ollama 正在运行；远程 provider 请检查网络。',
  'error.TIMEOUT': 'provider 响应超时。本地小模型在长提示下较慢，可重试或换用更小的模型。',
  'error.RATE_LIMIT': 'provider 限流了。请稍后重试。',
  'error.QUOTA': '该账号的额度已用尽。',
  'error.CONTEXT_WINDOW_EXCEEDED': '本次会话超出了模型的上下文长度。请开新会话，或换用上下文更大的模型。',
...
export const en = {
  ...
  'error.MISSING_CREDENTIAL': 'This provider has no API key yet. Open Settings → Models and add one.',
  'error.INVALID_CREDENTIAL': 'The provider rejected this API key. Re-enter it in Settings → Models.',
  'error.UNKNOWN_MODEL': 'The selected model is not configured. Pick another one in the composer.',
  'error.TRANSPORT': 'Could not reach the provider. For a local model, check that Ollama is running; for a remote one, check the connection.',
  'error.TIMEOUT': 'The provider did not answer in time. A small local model is slow on long prompts — retry, or pick a smaller one.',
  'error.RATE_LIMIT': 'The provider is rate-limiting this account. Try again in a moment.',
  'error.QUOTA': 'This account is out of quota.',
  'error.CONTEXT_WINDOW_EXCEEDED': 'This session outgrew the model context window. Start a new session, or pick a model with a larger one.',
