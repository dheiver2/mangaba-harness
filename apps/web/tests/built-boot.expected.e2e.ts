...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  if (clientBuildValue('DSH_CLIENT_BUILD_PROFILE') === 'official') {
    ...
    expect(screen.queryByText('Mangaba Local Build')).toBeNull()
  } else {
    expect(document.querySelector('svg[viewBox="291 193 102 110"]')).not.toBeNull()
    ...
    screen.getByText('Mangaba Local Build')
