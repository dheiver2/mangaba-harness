...
export interface Config {
  /** Mangaba Harness home directory exposed as `DSH_HOME`; defaults to `$DSH_HOME` or `~/.dsh`. */
  dshHome?: string
}
```
...
export interface Config {
  ...
  /** Mangaba Harness config root. Defaults to `$DSH_HOME` or `~/.dsh`. */
  dshHome?: string
...
export interface Config {
  /** Include the fixed Mangaba Harness identity before the deployment persona (default true). */
